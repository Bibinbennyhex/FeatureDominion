...
def run_test():
    ...
    try:
        ...
        # compare rolling arrays + grid only (scalar columns may diverge by design).
        s111 = (
            summary
        ...
        diff_s_to_l = s111.exceptAll(l111).count()
        diff_l_to_s = l111.exceptAll(s111).count()
        _assert_true(
            diff_s_to_l == 0 and diff_l_to_s == 0,
            "same-month Case III (no-tail invariant) should match on rolling arrays/grid "
            "between summary and latest_summary",
        )
