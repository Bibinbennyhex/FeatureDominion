"""
Shared test utilities for running src/summary_inc_v4.py in local docker.
"""
...
REGRESSION_ROOT = os.path.abspath(os.path.join(TESTS_DIR, ".."))
PROJECT_ROOT = os.path.abspath(os.path.join(REGRESSION_ROOT, ".."))
PIPELINE_PATH = os.path.join(PROJECT_ROOT, "src", "summary_inc_v4.py")


def _load_main_pipeline():
    module_name = "_regression_summary_inc_v4"
    for existing_name in ("summary_inc_v4", "summary_inc", "_summary_inc_v4_shared"):
        existing = sys.modules.get(existing_name)
        if existing is not None:
            sys.modules[module_name] = existing
            sys.modules["summary_inc"] = existing
            sys.modules["summary_inc_v4"] = existing
            return existing
...
main_pipeline = _load_main_pipeline()  # noqa: E402
...
def load_main_test_config(namespace: str) -> Dict:
    ...
    config_path = os.path.join(REGRESSION_ROOT, "config_v4.json")
