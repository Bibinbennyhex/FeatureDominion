...
try:
    ...
    from v4_pipeline_selector import resolve_pipeline_path
...
def load_v4_as_summary_inc(cache_name: str) -> Any:
    ...
    shared = sys.modules.get(SHARED_V4_MODULE_CACHE)
    if shared is not None:
        sys.modules[cache_name] = shared
        sys.modules["summary_inc"] = shared
        sys.modules["summary_inc_v2_unbounded"] = shared
        sys.modules["summary_inc_v4"] = shared
        return shared

    v4_path = resolve_pipeline_path(repo_root(), default_script="src/summary_inc_v4.py")
    ...
    sys.modules[SHARED_V4_MODULE_CACHE] = module
