...
def test_v4_case3_hot_lane_fail_fast_missing_latest():
    repo_root = Path(__file__).resolve().parents[2]
    module = load_v4_as_summary_inc("_summary_inc_v4_case3_hot_lane_fail_fast_missing_latest")
    ...
    spark = tu.create_spark_session("v4_case3_hot_lane_fail_fast_missing_latest")
    config = tu.load_main_test_config("v4_case3_hot_lane_fail_fast_missing_latest")
    ...
    try:
        ...
        _assert_true(error_message is not None, "Expected fail-fast error for missing latest row")
        _assert_true("missing latest_summary rows" in error_message, f"Unexpected error: {error_message}")

        print("[PASS] test_v4_case3_hot_lane_fail_fast_missing_latest")
    finally:
        spark.stop()


if __name__ == "__main__":
    test_v4_case3_hot_lane_fail_fast_missing_latest()
    print("[PASS] test_v4_case3_hot_lane_fail_fast_missing_latest.py")
