...
AUDIT_TIER = os.environ.get("AUDIT_TIER", "nightly").strip().lower() or "nightly"
if AUDIT_TIER not in {"smoke", "nightly"}:
    raise ValueError(f"Unsupported AUDIT_TIER={AUDIT_TIER}. Expected 'smoke' or 'nightly'")
TEST_FILES = get_v4_tier_tests(AUDIT_TIER)
...
def main():
    ...
    validate_v4_unique_suite(tests_dir, tier=AUDIT_TIER)
