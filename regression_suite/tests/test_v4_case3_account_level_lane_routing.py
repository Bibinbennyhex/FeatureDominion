...
def _history_prefix_match(summary_row, latest_row):
    for col_name in HISTORY_COLS:
        ...
        _assert_true(
            summary_hist == latest_hist[:36],
            f"{col_name} summary should match latest prefix",
        )


def test_v4_case3_account_level_lane_routing():
    repo_root = Path(__file__).resolve().parents[2]
    module = load_v4_as_summary_inc("_summary_inc_v4_case3_account_level_lane_routing")
    ...
    spark = tu.create_spark_session("v4_case3_account_level_lane_routing")
    config = tu.load_main_test_config("v4_case3_account_level_lane_routing")
    ...
    try:
        ...
        source_rows = [
            ...
            # hot-only account
            tu.build_source_row(hot_acct, "2025-12", source_ts, balance=5120, actual_payment=512),
            # mixed account (hot+cold)
            tu.build_source_row(mixed_acct, "2025-11", source_ts, balance=6130, actual_payment=613),
            ...
            tu.build_source_row(anchor_acct, "2026-02", source_ts, balance=1000, actual_payment=100),
        ]
        ...
        _assert_true(
            spark.catalog.tableExists("execution_catalog.checkpointdb.case_3_latest_history_context_patch"),
            "Expected latest-history context patch table to exist",
        )
        ...
        _assert_true(hot_acct in history_patch_keys, "Hot-only account should be in context patch output")
        _assert_true(cold_acct not in history_patch_keys, "Cold-only account should not be in context patch output")
        _assert_true(
        ...
        _assert_true(
            cold_acct not in latest_month_patch_keys,
            "Cold-only account should be excluded from case_3_latest_month_patch",
        )
        ...
        for acct, month, expected_balance in [
            ...
            (hot_acct, "2025-12", 5120),
            (mixed_acct, "2025-11", 6130),
        ...
        # Prefix-array parity is guaranteed by dedicated contract tests.
        # Here we keep lane-routing focused and assert strict prefix only for hot-only account.
        ...
        print(
            "[PASS] Case III account-level lane routing verified "
